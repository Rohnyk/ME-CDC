...
void  main(void)
{
     ...
     //creation of the key using ftok function ("man ftok" to see the functions)
     ShmKEY = ftok(".", 'x');
     printf("Shmkey %d",ShmKEY);

     //create the shared memory using shmget , give permissions,key
     ...
     //attach the process to the memory with id 
     ...
     while (ShmPTR->status != FILLED)
          ; //client waits for the server to fill and make the status value "FILLED"
     printf("   Client found the data is ready...\n"); //CLinet reads the data filled by client
     printf("   Client found %d %d %d %d in shared memory...\n",
                ShmPTR->data[0], ShmPTR->data[1], 
                ShmPTR->data[2], ShmPTR->data[3]);

     ShmPTR->status = TAKEN; //client changes the status to "TAKEN"
     printf("   Client has informed server data have been taken...\n");
     shmdt((void *) ShmPTR); //client process detach frmo the shared memory
